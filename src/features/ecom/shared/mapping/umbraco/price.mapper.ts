...
export function mapUmbracoPrice(p: any): PriceDto {
	...
	if (p.showPricesWithVat !== undefined)
		mp.showPricesWithVat = p.showPricesWithVat;
	if (v.withTax !== undefined || p.value !== undefined)
		mp.price = v.withTax || p.value;
	...
	if (p.vatPercentFormatted !== undefined)
		mp.vatPercentFormatted = p.vatPercentFormatted;
