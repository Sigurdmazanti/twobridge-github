...
describe('getShopifyCustomerIdByToken', () => {
    ...
    it('should return customer ID on successful API call', async () => {
        ...
        jest.spyOn(httpService, 'post').mockImplementation(() => of(mirroredAxiosSuccess));

        const result = await getShopifyCustomerIdByToken(authHeader, httpService);
    ...
    it('should call handleError on API error', async () => {
        ...
        jest.spyOn(httpService, 'post').mockImplementation(() => throwError(() => mockError));
