...
describe('CredentialsController', () => {
	...
	describe('changePassword', () => {
		it('should call CredentialsService.changePassword with the correct token', async () => {
			...
			await controller.changePassword(mockAuthHeader, mockNewPassword, mockExpressResponse);

			expect(credentialsService.changePassword).toHaveBeenCalledWith(mockAuthHeader, mockNewPassword);

			expect(sendResponse).toHaveBeenCalledWith(mockExpressResponse, mockResponse);
		});
		...
		it('should throw an exception if CredentialsService.changePassword', async () => {
			...
			await expect(
				controller.changePassword(mockAuthHeader, mockNewPassword, mockExpressResponse),
