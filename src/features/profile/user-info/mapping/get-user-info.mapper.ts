...
export const mapDynamicwebGetUserInfo = (
	...
	if (response?.customerNumber !== undefined)
		r.customerNumber = response.customerNumber;
	if (response?.address !== undefined) r.address = response.address;
	if (response?.address2 !== undefined) r.address2 = response.address2;
	if (response?.houseNumber !== undefined)
		r.houseNumber = response.houseNumber;
	...
	if (response?.countryCode !== undefined)
		r.countryCode = response.countryCode;
