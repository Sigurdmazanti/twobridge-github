 ...
 */
export function detectShopifyErrors(response: any): { status: number, function: string, from?: string, message?: string } | null {
    ...
    if (response?.data) {
        ...
        if (functionName && response.data[functionName]) {
            ...
            if (Array.isArray(errors)) {
                const firstError = errors.find((error: any) => error.code) || errors?.[0];
                
                if (firstError) {
                    return {
                        status: getHttpCodeForShopifyError(firstError.code) || 400,
 ...
 */
export function getHttpCodeForShopifyError(errorCode: ShopifyErrorCode): number {
